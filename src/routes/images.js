...
router.get("/*", async (req, res, next) => {
  try {
    ...
    logger.info(
      `Image path: ${imagePath}, Dimensions: ${dimensions ? `${dimensions.width}x${dimensions.height}` : "original"}`,
    );
...
async function handleResizedImage(imagePath, dimensions, res) {
  try {
    ...
    // Resize image
    const format = imageService.extractFormatFromFilename(imagePath);
    const resizedBuffer = await imageService.resizeImage(originalBuffer, dimensions.width, dimensions.height, format);
